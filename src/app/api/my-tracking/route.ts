...
export async function GET(request: Request) {
  ...
  try {
    ...
    const tripsWithGps = await Promise.all(
      activeTrips.map(async (trip: any) => {
        ...
        return {
          ...
          driver: trip.driver ? `${trip.driver.firstName || ""} ${trip.driver.lastName || ""}`.trim() || "Unknown" : "Unknown",
    ...
    return NextResponse.json({ message: error.message || "Failed to fetch tracking data" }, { status: 500 })
  }
}
